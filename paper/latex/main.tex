...
L24--30 rather than a single-layer gate. In plain terms: if you could stop the model at
...
\emph{generic desirability gate} or something with more structure. We test this three ways on
the model's own desirability axis (a control-vector direction fitted from prosocial/self-worth
vs.\ psychopathy/depression anchor items): two correlational tests
(Figure~\ref{fig:desirability}) and one causal intervention (Figure~\ref{fig:knockout}). All
three say the simple version is wrong:
...
machinery (lens invariance; no gain increase under the organism's own lens). The knockout adds
the causal half of this claim: the mask is not an online computation one could intercept ---
reversing the most natural candidate signal at the masking depth, at five times physiological
strength, leaves the denial intact. Yet the resulting
...
follow-up. The knockout steers one direction over one band (L30--34); a distributed,
non-linear, or multi-directional implementation of an online gate would evade it, so the
knockout rules out the single-axis version specifically.
